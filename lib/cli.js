...
import {maxSeed, normalizeSeed} from './randomize.js';
...
const FLAGS = {
	...
	randomize: {
		coerce: coerceLastValue,
		description: 'Run test files and concurrent tests in random order',
		type: 'boolean',
	},
	seed: {
		coerce: coerceLastValue,
...
export default async function loadCli() { // eslint-disable-line complexity
	...
	if (Object.hasOwn(conf, 'randomize') && typeof conf.randomize !== 'boolean') {
		exit('’randomize’ must be true or false.');
	}

	let randomSeed;
	if (combined.seed !== undefined) {
		try {
			randomSeed = normalizeSeed(combined.seed);
		} catch {
			exit(`The --seed flag and seed configuration must be an integer between 1 and ${maxSeed}.`);
		}
	}
	...
	const api = new Api({
		...
		nodeArguments,
		parallelRuns,
		randomize: combined.randomize === true || randomSeed !== undefined,
		randomSeed,
