...
function createCleanupChain(name, fn, defaults, beforeType) {
	const cleanup = startChain(name, (metadata, args) => {
		fn({...metadata, always: false, type: beforeType}, [...args]);
		fn({...metadata, always: true, type: 'after'}, [...args]);
	}, defaults);
...
export default function createChain(fn, defaults, meta) {
	...
	root.cleanup = createCleanupChain('test.cleanup', fn, {...defaults}, 'before');
	root.cleanupEach = createCleanupChain('test.cleanupEach', fn, {...defaults}, 'beforeEach');
	...
	root.serial.cleanup = createCleanupChain('test.serial.cleanup', fn, {...defaults, serial: true}, 'before');
	root.serial.cleanupEach = createCleanupChain('test.serial.cleanupEach', fn, {...defaults, serial: true}, 'beforeEach');
