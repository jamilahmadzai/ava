...
import {seedForString, shuffle} from './randomize.js';
...
export default class Runner extends Emittery {
	...
	getTestsToRunConcurrently(concurrentTests) {
		if (this.randomSeed === undefined) {
			return concurrentTests;
		}

		return shuffle(concurrentTests, seedForString(this.randomSeed, this.file));
	}
	...
	async start() {
		...
		const testsToRunConcurrently = this.getTestsToRunConcurrently(concurrentTests);
		...
		const concurrentPromise = Promise.all([beforePromise, serialPromise]).then(async ([beforeHooksOk, serialOk]) => {
			...
			const allOkays = await Promise.all(testsToRunConcurrently.map(task => this.runTest(task, contextRef.copy())));
